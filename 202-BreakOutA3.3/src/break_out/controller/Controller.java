...
public class Controller implements ActionListener, KeyListener {
	...
	public void keyPressed(KeyEvent e) {
		...
		if(e.getKeyCode()==KeyEvent.VK_RIGHT || e.getKeyCode()==KeyEvent.VK_D) {
			if((game.getLevel().getPaddleBottom().getPosition().getX() >=0 && game.getLevel().getPaddleBottom().getPosition().getX() <=Constants.SCREEN_WIDTH)||
			   (game.getLevel().getPaddleTop().getPosition().getX() >=0 && game.getLevel().getPaddleTop().getPosition().getX() <=Constants.SCREEN_WIDTH)) {
				
					if(game.getLevel().getBall().getPosition().getY() <= Constants.SCREEN_HEIGHT/2) {
